...
class ProductPage(BasePage):
    ...
    def price_of_book_added_to_basket(self):
        ...
        assert product_price == info_price, "Wrong price!!!!"
        print(f'Price in the info is: {info_price}')
        print(f'Product price is: {product_price}')

    def should_not_be_success_message(self):
        assert self.is_not_element_present(*ProductpageLocators.INFO_ABOUT_NEW_BOOK_IN_BASKET), \
            "Success message is presented, but should not be"

    def should_disappear_succes_messagge(self):
        assert self.is_disappeared(*ProductpageLocators.INFO_ABOUT_NEW_BOOK_IN_BASKET), \
            "Success message is still presented but should not be!"
